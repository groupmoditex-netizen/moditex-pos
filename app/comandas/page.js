...
function ComandasInner() {
  ...
  const [draftToLoad, setDraftToLoad] = useState(null); // draft obj to pass into ModalNueva
  ...
  useEffect(() => {
    ...
    // Listen for draft changes from ModalNueva
  ...
  function abrirDraft(draft) {
    setDraftToLoad(draft);
    setModal('nueva');
    setShowDrafts(false);
  }

  function descartarDraft(id) {
    const updated = loadDrafts().filter(d => d.id !== id);
    saveDrafts(updated);
    setDrafts(updated);
  }

  function descartarTodosDrafts() {
    if (!window.confirm('¿Descartar todas las comandas en espera?')) return;
    saveDrafts([]);
    setDrafts([]);
  }
        ...
        <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
          ...
          {drafts.length > 0 && (
            <button onClick={()=>setShowDrafts(v=>!v)}
              style={{padding:'9px 14px',background:showDrafts?'rgba(245,158,11,.12)':'none',border:'1px solid rgba(245,158,11,.5)',cursor:'pointer',fontFamily:'Poppins,sans-serif',fontSize:'11px',fontWeight:700,color:'#f59e0b',display:'flex',alignItems:'center',gap:'7px',transition:'all .15s',position:'relative'}}>
              ⏸ En Espera
              <span style={{background:'#f59e0b',color:'#000',borderRadius:'10px',padding:'1px 7px',fontSize:'10px',fontWeight:800}}>{drafts.length}</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div style={{marginBottom:'14px',background:'var(--surface)',border:'1px solid rgba(245,158,11,.3)',borderLeft:'3px solid #f59e0b',overflow:'hidden'}}>
          <div style={{padding:'10px 14px',background:'rgba(245,158,11,.06)',borderBottom:'1px solid rgba(245,158,11,.15)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
              ...
              <span style={{fontFamily:'DM Mono,monospace',fontSize:'9px',fontWeight:700,letterSpacing:'.14em',textTransform:'uppercase',color:'#f59e0b'}}>
                Comandas en espera — {drafts.length}
              </span>
            </div>
            <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
              <button onClick={descartarTodosDrafts}
                style={{padding:'4px 10px',background:'none',border:'1px solid rgba(217,30,30,.3)',cursor:'pointer',fontFamily:'DM Mono,monospace',fontSize:'8px',color:'var(--red)',letterSpacing:'.08em'}}>
                Descartar todas
              </button>
              <button onClick={()=>setShowDrafts(false)}
                style={{width:'24px',height:'24px',background:'none',border:'none',cursor:'pointer',color:'#aaa',fontSize:'14px'}}>✕</button>
            </div>
          </div>
          {drafts.map(draft => (
            <div key={draft.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'11px 14px',borderBottom:'1px solid var(--border)'}}>
              <div style={{width:'34px',height:'34px',borderRadius:'50%',background:'rgba(245,158,11,.1)',border:'1px solid rgba(245,158,11,.25)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',flexShrink:0}}>⏸</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:'Poppins,sans-serif',fontSize:'13px',fontWeight:700,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                  {draft.cliQuery || 'Sin cliente'}
                </div>
                <div style={{fontFamily:'DM Mono,monospace',fontSize:'8.5px',color:'#888',marginTop:'2px',display:'flex',gap:'10px',flexWrap:'wrap'}}>
                  <span>{draft.items?.length || 0} prenda{draft.items?.length !== 1 ? 's' : ''}</span>
                  {draft.items?.length > 0 && (
                    <span style={{color:'#aaa'}}>{draft.items.slice(0,3).map(i=>`${i.modelo}`).join(', ')}{draft.items.length > 3 ? `... +${draft.items.length-3}` : ''}</span>
                  )}
                  <span style={{color:'#c9a84c'}}>⏱ {draft.guardadoEn}</span>
                </div>
              </div>
              <div style={{display:'flex',gap:'6px',flexShrink:0}}>
                <button onClick={()=>abrirDraft(draft)}
                  style={{padding:'7px 16px',background:'#f59e0b',color:'#000',border:'none',cursor:'pointer',fontFamily:'Poppins,sans-serif',fontSize:'11px',fontWeight:700,whiteSpace:'nowrap'}}>
                  Continuar →
                </button>
                <button onClick={()=>descartarDraft(draft.id)}
                  style={{padding:'7px 10px',background:'none',border:'1px solid rgba(217,30,30,.25)',cursor:'pointer',fontFamily:'DM Mono,monospace',fontSize:'9px',color:'var(--red)'}}>
                  ✕
                </button>
              </div>
            </div>
